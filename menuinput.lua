...
function MenuInput:back(...)
	...
	if menu_component_manager ~= nil then
		...
		if chat_gui ~= nil and chat_gui._crimenet_chat_state then
			if chat_gui._enter_text_set then
				chat_gui._enter_text_set = false
				return --by pressing escape as said below "Not explicitly handling focus loss here as it is already handled properly by game code (ChatGui:esc_key_callback())"
			elseif chat_gui._is_crimenet_chat then
