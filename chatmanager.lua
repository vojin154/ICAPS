...
local function CreatePlayerStatesPanel(chatgui, parent_panel)
	...
	if alive(parent_panel:child("playerstates")) then
		parent_panel:remove(parent_panel:child("playerstates"))
	end
	...
	playerstates_panel:text({
		...
		blend_mode = "normal",
		visible = false --keeping it just incase i have plans for it in the future rn it gets cutoff with the search bar and i dont think this piece of text is important
	})
